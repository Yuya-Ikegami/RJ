...
class Cannon:
    ...
    def create_cannon(self, canvas: tk.Canvas):
        self.top2 = canvas.create_rectangle(self.x1, self.y1, self.x1+50, self.y1+40, fill = self.color)
        self.bottom2 = canvas.create_rectangle(self.x2, self.y2, self.x2+130, self.y2+40, fill = self.color)
